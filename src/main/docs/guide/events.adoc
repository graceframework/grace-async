...
If you wish to use a popular async framework such as RxJava3 as the `EventBus` implementation then you will need to add the appropriate dependency. For example for RxJava 3.x:

[source,groovy,subs="attributes"]
.build.gradle
----
implementation "org.graceframework.events:grace-events-rxjava3:{version}"
----
